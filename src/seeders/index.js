...
const createSystemAdmin = async (roles) => {
...
const seedDatabase = async () => {
  try {
    ...
    // Create a system user first for audit trail
    let systemUser;
    try {
      systemUser = await User.findOne({ email: 'system@seeder.internal' });
    ...
    // 1. Seed permissions
    const permissions = await seedPermissions(systemUser);
    
    // 2. Seed roles
    const roles = await seedRoles(permissions, systemUser);
    
    // 3. Create system administrator
    const adminUser = await createSystemAdmin(roles);
    
    // 4. Clean up temporary system user
    try {
      if (systemUser.email === 'system@seeder.internal') {
        await User.findByIdAndDelete(systemUser._id);
      }
    } catch (error) {
      logger.warn('Could not delete temporary system user:', error.message);
    }
...
const updatePermissions = async () => {
  try {
    ...
    // Find or create a system user for updates
    let systemUser = await User.findOne({ email: 'admin@system.com' });
    if (!systemUser) {
      logger.error('No admin user found for updates');
      return;
    }
